...
def strafe_left(body_height):
    cmd = RobotCommandBuilder.synchro_velocity_command(v_x=0.0, v_y=VELOCITY_BASE_SPEED, v_rot=0.0, body_height=body_height)
    command_client.robot_command(cmd, end_time_secs=time.time()+VELOCITY_CMD_DURATION)
...
def turn_left(body_height):
    cmd = RobotCommandBuilder.synchro_velocity_command(v_x=0.0, v_y=0.0, v_rot=VELOCITY_BASE_ANGULAR, body_height=body_height)
    command_client.robot_command(cmd, end_time_secs=time.time()+VELOCITY_CMD_DURATION)
